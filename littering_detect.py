...
midas_model_type = "dpt_beit_large_384"  # Change this to your desired model type if needed
midas_model_path = default_models[midas_model_type] # Replace this with the actual path to your MiDaS model

midas_optimize = False  # Set to True if your model is optimized
midas_height = 384  # Desired height for input images
midas_square = True  # Whether to resize input images to a square shape
midas, transform, net_w, net_h = load_model(midas_device, midas_model_path, midas_model_type, midas_optimize, midas_height, midas_square)
...
def create_side_by_side(image, depth, grayscale):
    ...
    if grayscale:
        ...
        right_side = cv2.applyColorMap(np.uint8(normalized_depth), cv2.COLORMAP_TWILIGHT_SHIFTED)
...
def detect(opt):
    ...
    for j in range(nframes):
            ...
            if ret:
                ...
                if switch:
                    original_image_rgb = np.flip(img_copy, 2)  # in [0, 255] (flip required to get RGB)
                    image = transform({"image": original_image_rgb/255})["image"]
                    ...
                    violation, trash_lists = violation_detect(trash_store, car_store , depth_map)
                #print(violation)
                    if violation:
                        trash_list=[]
                        plate_list=[]
                    ...
                    cv2.imwrite(str(depth_output), depth_map)
                    cv2.imwrite(str(depth_output1), depth_map1)
